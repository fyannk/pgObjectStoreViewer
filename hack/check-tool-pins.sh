...
# these versions: a document that names v1.6.0 goes stale the first time this
# runs, and the bump would carry a lie in the same commit. Point at the
# Makefile variable instead.
#
...
#
# Not listed, and why: the Go toolchain is compared across go.mod,
# api/go.mod, and the Dockerfile by check-go-version.sh instead, and the
# Node pins in ci.yml name an LTS line on purpose. Those stay
# hand-maintained; this file is the record that the choice was made, not an
# oversight.
